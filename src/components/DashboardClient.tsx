...
function periodDays(p: { start: string; end: string } | null): number {
  if (!p) return 1;
  const d = (Date.parse(p.end) - Date.parse(p.start)) / 86400000;
...
/** 전주대비(직전 기간 대비) — 클릭하면 이전값→현재값 상세 토글 */
function WoW({
  curr,
  prev,
  fmt,
  ...
  const delta = has ? (curr - prev!) / prev! : 0;
  const up = curr >= (prev ?? 0);
...
export function DashboardClient({ data }: { data: DashboardData }) {
  ...
  const toggleChange = () => setShowChange((v) => !v);
      ...
      <header className="flex flex-wrap items-center justify-between gap-3 rounded-2xl bg-white p-6 shadow-sm">
        ...
        <div className="flex items-center gap-3">
          <span className="rounded-lg bg-slate-100 px-4 py-2 text-sm font-medium text-slate-600">
            {periodText}
            {days > 1 && (
              <span className="ml-1 text-xs text-slate-400">({days}일)</span>
            )}
          </span>
          <button
